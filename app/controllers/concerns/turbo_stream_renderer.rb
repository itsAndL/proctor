module TurboStreamRenderer
  ...
  included do
    ...
    def render_turbo_stream_update(options = {})
      ...
      streams << turbo_stream.replace('email-inviting', EmailInvitingComponent.new(assessment: @assessment)) if options[:update_email_inviting]
      ...
      streams << turbo_stream.replace('bulk-inviting', BulkInvitingComponent.new(assessment: @assessment)) if options[:update_bulk_inviting]
